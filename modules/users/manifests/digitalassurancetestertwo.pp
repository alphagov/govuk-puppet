# Creates the digitalassurancetestertwo user
class users::digitalassurancetestertwo {
  govuk_user { 'digitalassurancetestertwo':
    fullname => 'digitalassurancetester two',
    email    => 'digitalassurancetester.two@digital.cabinet-office.gov.uk',
    ssh_key  => 'ssh-rsa  user@computer',
  }
}
