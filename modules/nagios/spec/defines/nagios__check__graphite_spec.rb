  ...
  context 'when passing target which includes double quotes' do
    ...
    it 'should contain a nagios_check resource' do
      should contain_nagios__check('count_tigers').with(
        ...
        :graph_url                  => /^https:\/\/graphite\.monitoring\.zoo\.tld\/render\/\?width=\d+&height=\d+&target=summarize\(zoo\.\*\.tiger,%225minutes%22,%22max%22,true\)&target=alias\(dashed\(constantLine\(10\)\),%22warning%22\)&target=alias\(dashed\(constantLine\(20\)\),%22critical%22\)$/,
      )
    end
  end
