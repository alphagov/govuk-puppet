...
class govuk::apps::static( $port = 3013 ) {
  ...
  if ! $should_use_proxy_vhost {
    nginx::config::vhost::static { "static.${app_domain}":
      to                => "localhost:${port}",
      protected         => false,
      aliases           => ['calculators', 'calendars', 'smartanswers', 'static', 'frontend', 'designprinciples', 'licencefinder', 'tariff', 'efg', 'feedback', 'datainsight-frontend', 'businesssupportfinder', 'limelight', 'transaction-wrappers'],
      ssl_only          => true,
