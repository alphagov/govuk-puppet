...
describe 'govuk::logstream', :type => :define do
  ...
  context 'with default args, enable => true' do
    ...
    it 'should pass appropriate CLI args' do
      should contain_file(upstart_conf).with(
        :ensure  => 'present',
        :content => /\| govuk_logpipe --source-host camel.example.com --bulk$/,
      )
    end
  end
  ...
  context 'with tags => Array' do
    ...
    it 'should pass --tags with list' do
      should contain_file(upstart_conf).with(
        :ensure  => 'present',
        :content => /\| govuk_logpipe --source-host camel.example.com --bulk -t zebra llama$/,
      )
    end
  end
  ...
  context 'with fields => Hash' do
    ...
    it 'should pass --fields with kv pairs' do
      should contain_file(upstart_conf).with(
        :ensure  => 'present',
        :content => /\| govuk_logpipe --source-host camel.example.com --bulk -f zebra=stripey llama=fluffy$/,
      )
    end
  end
  ...
  context 'with source_host => orangutan.zoo.com' do
    ...
    it 'should pass --fields with correct host kv' do
      should contain_file(upstart_conf).with(
        :ensure  => 'present',
        :content => /\| govuk_logpipe --source-host orangutan.zoo.com --bulk$/,
      )
    end
  end
  ...
  context 'with json => true' do
    ...
    it 'should pass --json arg' do
      should contain_file(upstart_conf).with(
        :ensure  => 'present',
        :content => /\| govuk_logpipe --json --source-host camel.example.com --bulk$/,
      )
    end
  end
end
